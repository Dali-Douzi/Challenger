...
router.post("/:chatId", protect, async (req, res) => {
  ...
  try {
    ...
    const senderUser = await User.findById(req.user.id).select(
      "username avatar"
    );
      ...
      recipients.map(async (teamId) => {
        ...
        if (io) {
          io.emit("newNotification", {
            teamId: teamId, // Only notify this specific team
