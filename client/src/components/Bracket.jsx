...
const ConnectionPoint = ({ type, position, componentId, pointId, isActive, onClick, color = '#3b82f6' }) => {
  return (
    <div
  ...
  return (
    <div
      ...
      style={{
        ...
        gap: '8px',
        padding: '8px',
        backgroundColor: isOver ? 'rgba(59, 130, 246, 0.2)' : 'rgba(71, 85, 105, 0.4)',
        borderRadius: '6px',
        border: isOver ? '2px solid #3b82f6' : '2px solid rgba(148, 163, 184, 0.2)',
        minHeight: '40px',
        transition: 'all 0.2s',
        position: 'relative'
      }}
    >
      {/* Winner Trophy */}
      {isWinner && (
        <EmojiEvents sx={{ color: 'gold', fontSize: '16px' }} />
      )}
      
      {/* Team Name */}
      <div style={{ flex: 1, color: '#e2e8f0', fontSize: '0.875rem', fontWeight: '500' }}>
        {team && team.name ? (
          <span
      ...
      {showScore && (
        <input
          ...
          onChange={(e) => {
            const newScore = e.target.value === '' ? 0 : (parseInt(e.target.value) || 0)
...
const GroupComponent = ({ 
  ...
  const handleTeamDrop = (slotIndex, teamId) => {
    ...
    const currentSlots = Array.isArray(currentData.slots) ? [...currentData.slots] : []
  ...
  return (
    <div
      ...
      style={{
        ...
        top: position.y,
        width: '220px',
        backgroundColor: deleteMode ? 'rgba(239, 68, 68, 0.2)' : 'rgba(30, 41, 59, 0.95)',
        border: deleteMode ? '2px solid #ef4444' : selectedForConnection ? '2px solid #3b82f6' : '2px solid #475569',
        borderRadius: '12px',
        padding: '16px',
      ...
      onClick={(e) => {
        if (deleteMode) {
          e.stopPropagation()
          console.log('Deleting group component:', id)
          onDelete(id)
        }
      }}
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <Typography variant="subtitle2" sx={{ color: '#e2e8f0', fontWeight: '600' }}>
          {(currentData && currentData.name) ? String(currentData.name) : `Group ${id.slice(-4)}`}
        </Typography>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {Array.from({ length: (currentData && typeof currentData.slotCount === 'number') ? currentData.slotCount : 4 }, (_, i) => {
          const slots = Array.isArray(currentData.slots) ? currentData.slots : []
          const slot = slots[i] || null
          const team = slot && slot.teamId ? participants.find(p => String(p._id || p.id) === String(slot.teamId)) : null
          return (
            <TeamSlot
              key={i}
              id={`${id}-slot-${i}`}
      ...
      <ConnectionPoint
        type="bottom"
        position={{ left: '50%', bottom: '-7px', transform: 'translateX(-50%)' }}
        componentId={id}
        pointId="bottom"
...
const SlotComponent = ({ 
  ...
  const currentData = data || {}
  const team = currentData.teamId ? participants.find(p => String(p._id || p.id) === String(currentData.teamId)) : null
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (deleteMode) {
          e.stopPropagation()
          console.log('Deleting slot component:', id)
          onDelete(id)
        }
      }}
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <Typography variant="subtitle2" sx={{ color: '#e2e8f0', fontWeight: '600' }}>
          {(currentData && currentData.name) ? String(currentData.name) : `Slot ${id.slice(-4)}`}
        </Typography>
        <IconButton size="small" onClick={(e) => { e.stopPropagation(); onEdit(id) }}>
          <Edit sx={{ fontSize: '16px', color: '#94a3b8' }} />
        </IconButton>
...
const MatchComponent = ({ 
  ...
  const currentData = data || {}
  const teamA = currentData.teamA ? participants.find(p => String(p._id || p.id) === String(currentData.teamA)) : null
  const teamB = currentData.teamB ? participants.find(p => String(p._id || p.id) === String(currentData.teamB)) : null
  
  // Determine winner
  const scoreA = Number(currentData.scoreA) || 0
  const scoreB = Number(currentData.scoreB) || 0
  const hasScores = (currentData.scoreA !== undefined && currentData.scoreA !== null) && (currentData.scoreB !== undefined && currentData.scoreB !== null)
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (deleteMode) {
          e.stopPropagation()
          console.log('Deleting match component:', id)
          onDelete(id)
        }
      }}
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <Typography variant="subtitle2" sx={{ color: '#e2e8f0', fontWeight: '600' }}>
          {(currentData && currentData.name) ? String(currentData.name) : `Match ${id.slice(-4)}`}
        </Typography>
        <IconButton size="small" onClick={(e) => { e.stopPropagation(); onEdit(id) }}>
          <Edit sx={{ fontSize: '16px', color: '#94a3b8' }} />
        </IconButton>
      ...
      {/* VS */}
      <div style={{ textAlign: 'center', margin: '8px 0', color: '#64748b', fontSize: '0.75rem', fontWeight: 'bold' }}>
        VS
      </div>
...
const ConnectionLine = ({ connection, components, onDelete, deleteMode }) => {
  ...
  const getConnectionPoint = (comp, pointId) => {
    ...
    if (comp.type === 'group') {
      const slotCount = (comp.data && typeof comp.data.slotCount === 'number') ? comp.data.slotCount : 4
  ...
  const createOptimalPath = (from, to) => {
    ...
    const isFromBottom = connection.from.pointId === 'loser' || connection.from.pointId === 'bottom'
    const isFromGreen = connection.from.pointId === 'winner' || connection.from.pointId === 'out'
    const isToBlueInput = connection.to.pointId === 'in' || connection.to.pointId === 'inA' || connection.to.pointId === 'inB'
    ...
    >
      ...
      <path
        ...
        style={{ 
          ...
          pointerEvents: deleteMode ? 'stroke' : 'none' // Only capture clicks on the actual stroke
        }}
        onClick={(e) => {
          if (deleteMode) {
            e.stopPropagation()
      ...
      <polygon
        ...
        style={{ 
          cursor: deleteMode ? 'pointer' : 'default',
          pointerEvents: deleteMode ? 'fill' : 'none' // Only capture clicks on the filled arrow
        }}
        onClick={(e) => {
          if (deleteMode) {
            e.stopPropagation()
            console.log('Deleting connection via arrow:', connection.id)
            onDelete(connection.id)
          }
        }}
      />
      ...
      {deleteMode && (
        <path
          ...
          onClick={(e) => {
            e.stopPropagation()
            console.log('Deleting connection via wide area:', connection.id)
            onDelete(connection.id)
          }}
        />
      )}
      ...
      {process.env.NODE_ENV === 'development' && (
        <>
          <circle cx={fromPoint.x} cy={fromPoint.y} r="3" fill="red" opacity="0.7" />
          <circle cx={toPoint.x} cy={toPoint.y} r="3" fill="blue" opacity="0.7" />
        </>
      )}
    </svg>
  )
...
const Bracket = ({ participants = [], organizerMode = false, tournament = {} }) => {
  ...
  const [editDialogOpen, setEditDialogOpen] = useState({ group: false, slot: false, match: false })
  ...
  const getUnplacedTeams = () => {
    ...
    return participants.filter(team => !placedTeamIds.has(String(team._id || team.id)))
  }
  ...
  useEffect(() => {
    const id = tournament && ('_id' in tournament) ? String(tournament._id).trim() : ''
    const tournamentId = id || 'default'
    const saved = localStorage.getItem(`bracket-${tournamentId}`)
    if (saved) {
      try {
        const parsed = JSON.parse(saved)
        const savedComponents = Array.isArray(parsed.components) ? parsed.components : []
        const savedConnections = Array.isArray(parsed.connections) ? parsed.connections : []
        dispatchComponents({ type: 'SET_COMPONENTS', payload: savedComponents })
        dispatchConnections({ type: 'SET_CONNECTIONS', payload: savedConnections })
      } catch (e) {
  ...
  useEffect(() => {
    const tournamentId = tournament && typeof tournament === 'object' && '_id' in tournament ? String(tournament._id) : 'default'
    localStorage.setItem(`bracket-${tournamentId}`, JSON.stringify({ 
        ...
        if (hasWinner) {
          ...
          connections.forEach(conn => {
            if (conn.from && conn.from.componentId === id) {
              const targetComp = components.find(c => c.id === conn.to.componentId)
              if (targetComp) {
  ...
  const handleComponentDelete = (id) => {
    console.log('handleComponentDelete called with id:', id)
    saveToHistory()
    
    // Delete connections first using simple filter
    connections.forEach(conn => {
  ...
  const handleConnectionDelete = (connectionId) => {
    console.log('handleConnectionDelete called with id:', connectionId)
    saveToHistory()
  ...
  const handleUndo = () => {
    if (historyIndex > 0) {
      ...
      dispatchComponents({ type: 'SET_COMPONENTS', payload: prevState.components })
      dispatchConnections({ type: 'SET_CONNECTIONS', payload: prevState.connections })
  ...
  const handleRedo = () => {
    if (historyIndex < history.length - 1) {
      ...
      dispatchComponents({ type: 'SET_COMPONENTS', payload: nextState.components })
      dispatchConnections({ type: 'SET_CONNECTIONS', payload: nextState.connections })
      setHistoryIndex(historyIndex + 1)
    }
  }
  ...
  const handleCanvasMouseDown = (e) => {
    ...
    if (e.target.id === 'bracket-canvas' || e.target.classList.contains('canvas-background')) {
      setIsPanning(true)
        ...
        if (dropId.indexOf('-slot') !== -1 || dropId.indexOf('-team') !== -1) {
          ...
          if (component) {
            ...
            if (component.type === 'group') {
              ...
              const newSlots = Array.isArray(component.data.slots) ? [...component.data.slots] : []
              // Ensure array is large enough
              while (newSlots.length <= slotIndex) {
                newSlots.push(null)
              }
              newSlots[slotIndex] = { teamId, score: 0 }
              handleComponentUpdate(componentId, { ...component.data, slots: newSlots })
    ...
    >
      <Box sx={{ mb: 6 }}>
        ...
        <Box sx={{ mb: 3, display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <Button
            ...
            sx={{ 
              ...
              borderColor: '#4ade80',
              backgroundColor: connectionMode === 'winner' ? '#4ade80' : 'transparent'
            }}
          >
            🟢 Winner
          </Button>
          <Button
            ...
            sx={{ 
              ...
              borderColor: '#ef4444',
              backgroundColor: connectionMode === 'loser' ? '#ef4444' : 'transparent'
            }}
          >
            🔴 Loser
          </Button>
          <Button
            ...
            sx={{ 
              ...
              borderColor: '#3b82f6',
              backgroundColor: connectionMode === 'normal' ? '#3b82f6' : 'transparent'
            }}
          >
            🔵 Link
          </Button>
          ...
          <Button
            ...
            startIcon={<Delete />}
            onClick={() => {
              console.log('Delete button clicked, deleteMode was:', deleteMode) // Debug log
        ...
        {unplacedTeams.length > 0 && (
          <Paper sx={{ mb: 3, p: 2, bgcolor: "background.paper" }} elevation={3}>
        ...
        >
          <div
            ...
            style={{
              ...
              transform: `scale(${zoom}) translate(${panOffset.x}px, ${panOffset.y}px)`,
              transformOrigin: 'center center',
              cursor: deleteMode ? 'crosshair' : (isPanning ? 'grabbing' : 'grab')
            }}
          >
            {/* Grid */}
            <div
        ...
        <EditGroupDialog
          ...
          onClose={() => { setEditDialogOpen({ group: false, slot: false, match: false }); setEditingComponent(null) }}
